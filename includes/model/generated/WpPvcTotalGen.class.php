...
<?php
	/**
	 * The abstract WpPvcTotalGen class defined here is
	 * code-generated and contains all the basic CRUD-type functionality as well as
	 * basic methods to handle relationships and index-based loading.
	 *
	 * To use, you should use the WpPvcTotal subclass which
	 * extends this WpPvcTotalGen class.
	 *
	 * Because subsequent re-code generations will overwrite any changes to this
	 * file, you should leave this file unaltered to prevent yourself from losing
	 * any information or code changes.  All customizations should be done by
	 * overriding existing or implementing new methods, properties and variables
	 * in the WpPvcTotal class.
	 *
	...
	class WpPvcTotalGen extends QBaseClass implements IteratorAggregate {
		...
		/**
		 * Protected member variable that maps to the database PK Identity column wp_pvc_total.id
		 * @var integer intId
		 */
		protected $intId;
		...
		/**
		 * Protected member variable that maps to the database column wp_pvc_total.postnum
		...
		/**
		 * Protected member variable that maps to the database column wp_pvc_total.postcount
		...
		{
			$this->intId = WpPvcTotal::IdDefault;
			$this->strPostnum = WpPvcTotal::PostnumDefault;
			$this->intPostcount = WpPvcTotal::PostcountDefault;
		}
		...
		/**
		 * Load a WpPvcTotal from PK Info
		 * @param integer $intId
		...
		public static function Load($intId, $objOptionalClauses = null) {
			...
			if (QApplication::$objCacheProvider && !$objOptionalClauses && QApplication::$Database[2]->Caching) {
				$strCacheKey = QApplication::$objCacheProvider->CreateKey(QApplication::$Database[2]->Database, 'WpPvcTotal', $intId);
			...
			$objToReturn = WpPvcTotal::QuerySingle(
				QQ::AndCondition(
		...
		public static function LoadAll($objOptionalClauses = null) {
			...
			// Call WpPvcTotal::QueryArray to perform the LoadAll query
			try {
				return WpPvcTotal::QueryArray(QQ::All(), $objOptionalClauses);
		...
		/**
		 * Count all WpPvcTotals
		 * @return int
		 */
		public static function CountAll() {
			// Call WpPvcTotal::QueryCount to perform the CountAll query
			return WpPvcTotal::QueryCount(QQ::All());
		}
		...
		protected static function BuildQueryStatement(&$objQueryBuilder, QQCondition $objConditions, $objOptionalClauses, $mixParameterArray, $blnCountOnly) {
			...
			// Create/Build out the QueryBuilder object with WpPvcTotal-specific SELET and FROM fields
			$objQueryBuilder = new QQueryBuilder($objDatabase, 'wp_pvc_total');
			...
			if ($blnAddAllFieldsToSelect) {
				WpPvcTotal::GetSelectFields($objQueryBuilder, null, QQuery::extractSelectClause($objOptionalClauses));
			}
			$objQueryBuilder->AddFromItem('wp_pvc_total');
		...
		/**
		 * Static Qcubed Query method to query for a single WpPvcTotal object.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		 * @return WpPvcTotal the queried object
		 */
		public static function QuerySingle(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			// Get the Query Statement
			try {
				$strQuery = WpPvcTotal::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, false);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}

			// Perform the Query, Get the First Row, and Instantiate a new WpPvcTotal object
			...
			if ($objQueryBuilder->ExpandAsArrayNodes) {
				...
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = WpPvcTotal::InstantiateDbRow($objDbRow, null, $objQueryBuilder->ExpandAsArrayNodes, $objToReturn, $objQueryBuilder->ColumnAliasArray);
					if ($objItem)
						$objToReturn[] = $objItem;
				}
				...
				if(null === $objDbRow)
					return null;
				return WpPvcTotal::InstantiateDbRow($objDbRow, null, null, null, $objQueryBuilder->ColumnAliasArray);
			}
		}

		/**
		 * Static Qcubed Query method to query for an array of WpPvcTotal objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		 * @return WpPvcTotal[] the queried objects as an array
		 */
		public static function QueryArray(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			...
			$objDbResult = $objQueryBuilder->Database->Query($strQuery);
			return WpPvcTotal::InstantiateDbResult($objDbResult, $objQueryBuilder->ExpandAsArrayNodes, $objQueryBuilder->ColumnAliasArray);
		}
		...
		/**
		 * Static Qcubed Query method to query for a count of WpPvcTotal objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClausees additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		public static function QueryCount(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			// Get the Query Statement
			try {
				$strQuery = WpPvcTotal::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, true);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		public static function QueryArrayCached(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null, $blnForceUpdate = false) {
			...
			$objCache = new QCache('qquery/wppvctotal', $strQuery);
			$cacheData = $objCache->GetData();

			if (!$cacheData || $blnForceUpdate) {
				$objDbResult = $objQueryBuilder->Database->Query($strQuery);
				$arrResult = WpPvcTotal::InstantiateDbResult($objDbResult, $objQueryBuilder->ExpandAsArrayNodes, $objQueryBuilder->ColumnAliasArray);
		...
		/**
		 * Updates a QQueryBuilder with the SELECT fields for this WpPvcTotal
		...
		/**
		 * Instantiate a WpPvcTotal from a Database Row.
		 * Takes in an optional strAliasPrefix, used in case another Object::InstantiateDbRow
		 * is calling this WpPvcTotal::InstantiateDbRow in order to perform
		...
		*/
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $strExpandAsArrayNodes = null, $arrPreviousItems = null, $strColumnAliasArray = array()) {
			...
			// Create a new instance of the WpPvcTotal object
		...
		/**
		 * Instantiate an array of WpPvcTotals from a Database Result
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $strExpandAsArrayNodes = null, $strColumnAliasArray = null) {
			...
			if ($strExpandAsArrayNodes) {
				$objToReturn = array();
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = WpPvcTotal::InstantiateDbRow($objDbRow, null, $strExpandAsArrayNodes, $objToReturn, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
					}
				}
				...
				while ($objDbRow = $objDbResult->GetNextRow())
					$objToReturn[] = WpPvcTotal::InstantiateDbRow($objDbRow, null, null, null, $strColumnAliasArray);
			}
		...
		/**
		 * Instantiate a single WpPvcTotal object from a query cursor (e.g. a DB ResultSet).
		 * Cursor is automatically moved to the "next row" of the result set.
		 * Will return NULL if no cursor or if the cursor has no more rows in the resultset.
		 * @param QDatabaseResultBase $objDbResult cursor resource
		 * @return WpPvcTotal next row resulting from the query
		 */
		public static function InstantiateCursor(QDatabaseResultBase $objDbResult) {
			...
			return WpPvcTotal::InstantiateDbRow($objDbRow, null, $strExpandAsArrayNodes, null, $strColumnAliasArray);
		}
		...
		/**
		 * Load a single WpPvcTotal object,
		 * by Id Index(es)
		 * @param integer $intId
		 * @param QQClause[] $objOptionalClauses additional optional QQClause objects for this query
		 * @return WpPvcTotal
		...
		*/
		...
		public function Save($blnForceInsert = false, $blnForceUpdate = false) {
			...
			try {
				if ((!$this->__blnRestored) || ($blnForceInsert)) {
					...
					$mixToReturn = $this->intId = $objDatabase->InsertId('wp_pvc_total', 'id');
				} else {
		...
		/**
		 * Delete this WpPvcTotal
		 * @return void
		 */
		public function Delete() {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Cannot delete this WpPvcTotal with an unset primary key.');
        ...
        /**
 	     * Delete this WpPvcTotal ONLY from the cache
 		 * @return void
		 */
		public function DeleteCache() {
			if (QApplication::$objCacheProvider && QApplication::$Database[2]->Caching) {
				$strCacheKey = QApplication::$objCacheProvider->CreateKey(QApplication::$Database[2]->Database, 'WpPvcTotal', $this->intId);
				QApplication::$objCacheProvider->Delete($strCacheKey);
			}
		}

		/**
		 * Delete all WpPvcTotals
		 * @return void
		 */
		...
		/**
		 * Reload this WpPvcTotal from the database.
		 * @return void
		 */
		public function Reload() {
			// Make sure we are actually Restored from the database
			if (!$this->__blnRestored)
				throw new QCallerException('Cannot call Reload() on a new, unsaved WpPvcTotal object.');
		...
		public static function GetDatabaseName() {
			return QApplication::$Database[WpPvcTotal::GetDatabaseIndex()]->Database;
		}
		...
		public static function GetArrayFromSoapArray($objSoapArray) {
			...
			foreach ($objSoapArray as $objSoapObject)
				array_push($objArrayToReturn, WpPvcTotal::GetObjectFromSoapObject($objSoapObject));
     ...
     **/
	class QQNodeWpPvcTotal extends QQNode {
		protected $strTableName = 'wp_pvc_total';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'WpPvcTotal';
		public function __get($strName) {
			switch ($strName) {
				case 'Id':
     ...
     * @property-read QQNode $_PrimaryKeyNode
     **/
	class QQReverseReferenceNodeWpPvcTotal extends QQReverseReferenceNode {
		protected $strTableName = 'wp_pvc_total';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'WpPvcTotal';
		public function __get($strName) {
